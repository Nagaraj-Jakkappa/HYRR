...
const socialLinks = [
  {
    label: 'LinkedIn',
    href: 'https://www.linkedin.com/company/joinhyrr',
    icon: (
  ...
  {
    label: 'Instagram',
    href: 'https://www.instagram.com/joinhyrr',
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
...
const FooterLink = ({ to, children }: { to: string, children: React.ReactNode }) => (
  <Link to={to} className="text-[#8b92a5] hover:text-white transition-colors text-[14px] leading-8 block font-medium">
    {children}
  </Link>
);
...
export default function Footer() {
  return (
    <footer className="bg-[#21242c] border-t border-white/[0.04] relative z-10 font-sans pt-16">
        ...
        <div className="flex items-center justify-start md:justify-end gap-3 pb-8">
             <div className="bg-white px-3 py-1.5 rounded-md flex items-center gap-2 shadow-sm">
                ...
                <div className="text-left leading-[1.1]">
                  <div className="text-[9px] font-black text-[#0eb563]">SECURE</div>
                  <div className="text-[7px] text-gray-500 font-bold tracking-tight">SSL ENCRYPTION</div>
                </div>
             </div>
             <div className="bg-white px-3 py-1.5 rounded-md flex items-center gap-2 shadow-sm">
                <ShieldCheck className="text-[#0eb563]" size={20} />
      ...
      {/* BOTTOM ROW */}
      <div className="bg-[#191b21] py-8 border-t border-white/5">
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex flex-col md:flex-row items-center justify-between gap-6">
          <p className="text-white text-[15px] font-medium">
            © {new Date().getFullYear()} <span className="font-bold">HYRR</span>. All rights reserved
          </p>
